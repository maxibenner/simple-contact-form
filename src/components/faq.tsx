...
export default function FAQ({ formId }: { formId?: string }) {
  ...
  async function handleSubmit(e: any) {
    ...
    const res = await fetch(
      `https://www.simplecontactform.org/submit/${formId}`,
      {
        method: "POST",
        body: formData,
      }
  ...
  return (
    <Accordion type="single" collapsible className="[&_*]:text-start">
      <AccordionItem value="item-1">
        <AccordionTrigger>
          How do you limit unwanted submissions?
        </AccordionTrigger>
        <AccordionContent>
          Alongside standard spam prevention techniques, we also use an LLM to
          filter out spam that other methods might miss.
        </AccordionContent>
      </AccordionItem>
      <AccordionItem value="item-2">
        ...
        <AccordionContent>
          If you have worked with HTML before, it should take no longer than 5
          minutes. Our personal record is 2 minutes!
        </AccordionContent>
      </AccordionItem>
      <AccordionItem value="item-3">
        <AccordionTrigger>Is it styled?</AccordionTrigger>
        <AccordionContent>
          No, we only provide the submission logic. You can style it however you
          like.
        </AccordionContent>
      </AccordionItem>
      {formId && (
